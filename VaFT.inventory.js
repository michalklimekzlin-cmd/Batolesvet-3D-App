// VaFT • Inventář (v7) — vždy vrací tipsFor() jako pole
console.log("✅ VaFT.inventory.js v7 načten");
...
export async function buildInventory(state = { mix:{}, label:"..." }) {
  ...
  items.push({ id:'core', title:`Jádro (${state.label||'…'})`, power: 1 });

  const tipsFor = (m={}) => {
    const tips=[];
    if (m.B  > 1.5) tips.push('🌱 Klid je síla – B roste.');
